...
| 드래그 | 사용자 설정은 남아도 새 libinput 대신 예전 사설 라이브러리가 계속 사용될 수 있음 | `status`에 `REBUILD REQUIRED`가 나오면 `./ubuntu-custom install drag` (맞는 보관본이 없으면 자동 재빌드) |
| Mutter | 더 높은 공식 패키지 버전이 로컬 패치를 교체할 수 있음 | 공식 수정 포함 여부 확인; 미포함이면 `./ubuntu-custom install mutter` 또는 수동 재적용 |
...
libinput 또는 Mutter가 업데이트되면 `./ubuntu-custom install drag` / `install mutter`가 보관 목록에서 맞는 버전을 고르고, 없으면 공식 소스를 받아 재빌드한다. 빌드 도구가 없으면 `./build-custom deps …`가 안내하는 명령을 먼저 실행한다. 자동 재빌드를 원하지 않으면 `--no-rebuild`를 쓴다. 공식 Mutter에 이미 수정이 포함되면 공식 패키지를 그대로 사용하며, 예전 `+keymapfix1` 문자열이 없어졌다는 이유만으로 되돌리지 않는다.

...
**2026-09-19 09:59 KST 후속:** 사용자의 추가 지시에 따라 sudo/sudo-i/polkit 관리자 지문 인증을 실제 적용했다. [현재 설정·재적용·원복 안내](../../docs/admin-fingerprint.md)를 사용한다. 아래는 최초 보관 시점의 기록이다.
...
최초 보관 때는 이 항목을 재설치 목록에서 제외했다. 이후 별도 도구 `admin-fingerprint`를 추가했다. `ubuntu-custom install fingerprint`는 계속 센서 드라이버만 설치하며, 관리자 PAM 인증은 `./admin-fingerprint install`로 별도 적용한다.
